...
// 处于发布阶段
    ...
    [request setValue:[KKCommon language] forHTTPHeaderField:@"Language"];
...
+ (void)uploadBugInfo:(NSString *)bug result:(ResultBlock)result{
    ...
    NSString *la = [[[NSUserDefaults standardUserDefaults] objectForKey:@"AppleLanguages"] firstObject];
...
//    NSString *deviceJson = [device jk_JSONString];
    ...
    NSURL *url = [NSURL URLWithString:@"https://receiver.dungeonsgames.com"];
...
+ (void)apiWithAppleID:(NSString *)appleID result:(ResultBlock)result{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/api/gameh5data.php?num=%d",YXNetAPI_Wap,arc4random() % 10000];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params addValue:appleID       key:@"appid"];
    [params addValue:APPVERSION    key:@"version"];
    [params addValue:@"appinfocry" key:@"act"];
    
    [manager GET:url parameters:params headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
//        responseObject = [YXNet AES128DecodeData:responseObject[@"data"]];
        
        NSInteger code = [responseObject[@"code"] integerValue];
        YXError *err = [YXError initWithCode:[NSString stringWithFormat:@"%ld",(long)code]];
        ...
        if (code == 0) {
            ...
            if (data == nil || [data isKindOfClass:[NSNull class]]) {
                ...
                BOOL isShow = [info[@"isShow"] boolValue];
                NSString *url = info[@"url"];
                if (isShow && url.length > 0) {
                    NSInteger timeInterval = [[NSDate date] timeIntervalSince1970];
                    url = [NSString stringWithFormat:@"%@&version=%@&timeInterval=%.0ld",url,APPVERSION,(long)timeInterval];
                    result(true,url,nil);
                }else{
                    err.describe = responseObject[@"msg"];
                    result(false,nil,err);
