...
@interface KKApiVC ()<WKUIDelegate,WKNavigationDelegate,WKScriptMessageHandler>

@property (nonatomic, copy) NSArray *funcs;

@end

@implementation KKApiVC
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([name isEqualToString:OVApiPassValue]) {
        ...
        NSString *text = [NSString stringWithFormat:@"%@",message.body];
        if (text.length > 0) {
            [KKManager copyWithText:text msg:LocalizedString(@"Message")];
        }
        ...
        // 统计
        NSString *price = [KKInfos sharedKKInfos].price;
...
- (void)loginOut{
    ...
    KKCancellationVC *vc = [[KKCancellationVC alloc] init];
    vc.vc = self;
    ...
    //            [YXStatis sdkOrder];
            ...
            if ([channel isEqualToString:@"5"] || [channel isEqualToString:@"11"] || channel == nil) {
                ...
                [self dismissViewControllerAnimated:true completion:^{
                    ...
                    [KKManager toPsyWithProductId:info.goodsID];
                }];
            } else {
                // 其他
                [self dismissViewControllerAnimated:true completion:^{
                    
                    KKApiVC *vc = [[KKApiVC alloc] init];
